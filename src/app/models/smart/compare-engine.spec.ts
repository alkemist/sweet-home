...
import {CompareEngine, CompareStateEnum, PanelEnum} from "./compare-engine";
import {JSONPath, JSONValue} from "@tools";
...
describe("CompareEngine", () => {
  ...
  const compareEngine = new CompareEngine((_: JSONPath) => {
    return "id";
  });

  compareEngine.update(PanelEnum.LEFT, jsonLeft);
  compareEngine.update(PanelEnum.RIGHT, jsonRight);
  ...
  it.each([
    ...
    (compareTest) => {
      expect(compareEngine.getState(PanelEnum.LEFT, compareTest.path.split("/"))).toBe(compareTest.expected);
    }
  );
  ...
  it.each([
    ...
    (compareTest) => {
      expect(compareEngine.getState(PanelEnum.RIGHT, compareTest.path.split("/"))).toBe(compareTest.expected);
    }
  );
});
